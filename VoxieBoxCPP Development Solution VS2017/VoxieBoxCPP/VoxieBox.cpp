...
VoxieBox::VoxieBox()
{
	...
	{
		MessageBox(0, "Error: Can't load or locate voxiebox.dll. Put voxiebox.dll in the current folder or check that voxiebox.dll is included in System's PATH variable", "", MB_OK); success = false;
	}
...
/**
 * @param indiceArray	the indiceArray to pass in
 * @param indiiceCount	the number of indices within the array
 */ 
void VoxieBox::indicesCpyXYZ_2_UVC(inds_t * indiceArray, int indiceCount)
{
	for (int i = 0; i < indiceCount; i++) { indiceArray[i].uvci = indiceArray[i].xyzi & ~EXT_START_LOOP; }
}
 ...
 */
void VoxieBox::drawQuad(const char* filename, point3d* pos, float width, float height, float hAng, float vAng, float twist, int col , float uValue , float vValue)
{
	poltex_t vt[4];
	point3d vx, vy;
	float f, g;
...
int VoxieBox::breath(voxie_inputs_t* input)
{
	...
	// now clipshape can be from either system
		...
		if (vw.clipshape == 0) 	voxie_drawbox(&vf, -vw.aspx + 1e-3f, -vw.aspy + 1e-3f, -vw.aspz, +vw.aspx - 1e-3f, +vw.aspy - 1e-3f, +vw.aspz, 1, drawBorderCol);
			...
			for (j = -64; j <= 64; j++)
			{
				...
				for (i = 0; i < n; i++)
				{
					voxie_drawlin(&vf, cos((float)(i + 0) * PI * 2.0 / (float)n) * (r), sin((float)(i + 0) * PI * 2.0 / (float)n) * (r), (float)j * vw.aspz / 64.f,
						cos((float)(i + 1) * PI * 2.0 / (float)n) * (r), sin((float)(i + 1) * PI * 2.0 / (float)n) * (r), (float)j * vw.aspz / 64.f, drawBorderCol);
				
					if (vw.asprmin == 0) continue;
					voxie_drawlin(&vf, cos((float)(i + 0) * PI * 2.0 / (float)n) * (rm), sin((float)(i + 0) * PI * 2.0 / (float)n) * (rm), (float)j * vw.aspz / 64.f,
						cos((float)(i + 1) * PI * 2.0 / (float)n) * (rm), sin((float)(i + 1) * PI * 2.0 / (float)n) * (rm), (float)j * vw.aspz / 64.f, drawBorderCol);
			...
			for (i = 0; i < n; i++)
			{
				voxie_drawlin(&vf, cos((float)(i + 0) * PI * 2.0 / (float)n) * (r), sin((float)(i + 0) * PI * 2.0 / (float)n) * (r), -vw.aspz,
					cos((float)(i + 0) * PI * 2.0 / (float)n) * (r), sin((float)(i + 0) * PI * 2.0 / (float)n) * (r), +vw.aspz, drawBorderCol);

			//	voxie_drawlin(&vf, cos((float)(i + 0) * PI * 2.0 / (float)n) * (rm), sin((float)(i + 0) * PI * 2.0 / (float)n) * (rm), -vw.aspz,
				//	cos((float)(i + 0) * PI * 2.0 / (float)n) * (rm), sin((float)(i + 0) * PI * 2.0 / (float)n) * (rm), +vw.aspz, drawBorderCol);
			}
			...
			for (j = -1; j <= 1; j += 2) { pp.z = vw.aspz * j; voxie_printalph_ext(&vf, &pp, &rr, &dd, .001f, drawBorderCol, "O"); }

			n = 32;
			for (i = 0; i < n; i++)
			{
				voxie_drawlin(&vf, cos((float)(i + 0) * PI * 2.0 / (float)n) * vw.aspr, sin((float)(i + 0) * PI * 2.0 / (float)n) * vw.aspr, -vw.aspz,
					cos((float)(i + 0) * PI * 2.0 / (float)n) * vw.aspr, sin((float)(i + 0) * PI * 2.0 / (float)n) * vw.aspr, +vw.aspz, drawBorderCol);
...
int VoxieBox::voxie_load(voxie_wind_t* vw)
{
	...
	voxie_menu_updateitem = (void(__cdecl*)(int, const char*, int, double))GetProcAddress(hvoxie, "voxie_menu_updateitem");
	...
	voxie_drawmeshtex = (void(__cdecl*)(voxie_frame_t*, const char*, poltex_t*, int, int*, int, int, int))GetProcAddress(hvoxie, "voxie_drawmeshtex");
	voxie_drawmeshtex_ext = (void(__cdecl*)(voxie_frame_t* vf, const char* fileName, point3d * xyz, int xyzn, uvcol_t * uvc, int uvcn, inds_t * inds, int ninds, int flags, int col))GetProcAddress(hvoxie, "voxie_drawmeshtex_ext");
...
/*
 * mode=0: full shutdown
 * mode=1: partial shutdown (when voxieos spawns app - quicker restart)
 * mode=2: kill audio on shutdown -> used when exiting when sound is being played.
 * 
 */ 
 ...
 *  Must be called between startFrame() & endFrame() functions.
 */
void VoxieBox::drawTextSimp(point3d* pos, float textWidth, float textHeight, float hang, float vang, float tilt, int col, const char* fmt, ...)
{
	va_list arglist;
	point3d vx, vy;
	float f, ch, sh, cv, sv;
 ...
 */
double VoxieBox::debugBar(int posx, int posy, double currentVal, double maxVal, double minVal, const char* text, int type) {
 ...
 */
void VoxieBox::menuAddTab(const char* st, int x, int y, int xs, int ys)
{
...
*/
void VoxieBox::menuAddItem(const char* label, int x, int y, int xSize, int ySize, int id, int type, int state, int col, double startingVal, double minVal, double maxVal, double minStepVal, double majStepVal)
{
 ...
 */
void VoxieBox::updateMenu(int id, const char* st, int state, double v)
{
...
*  @param  flags		mostly fill mode and texture settings +0:dots, +1:lines, +2:surfaces, +3:solid, +8:texnam is tiletype * instead of filename, +16 vertex shading 
*  @param  col			24-bit hexadecimal base color value (RGB) (0x404040) is the natural value
*
*			@example
*
...
*	Must be called between startFrame() & endFrame() functions.
*/
void VoxieBox::drawMesh(const char* fileName, poltex_t* verticeList, int verticeNum, int* meshList, int meshNum, int flags, int col = 0xffffff)
{
...
*  @param  fileNam 			texture filename or pointer to tiletype structure if (flags&8) or leave null for no texture
*							Be sure to fill .u and .v fields of poltex_t when using a texture.
*  @param  VerticeXYZArray	pointer to the vertice positional  array (as a point3d data type)
*  @param  verticeXYZNum	max number of vertice XYZ count (how long the array is)
*  @param  verticeUVC		pointer to the array of vertice UVI array (as a uvcol_t data type)
*  @param  verticeUVCNum	max number of vertice UVI count (how long the array is)
*  @param  indiceArray		pointer to the indice indexes (both XYZ and UVI) use .xyz + (1<<31) || .xyz + EXT_START_LOOP to signify the start of a new polygon loop.
*  @param  indiceNum		number of entries in indiceArray
*  @param  flags			mostly fill mode and texture settings +0:dots, +1:lines, +2:surfaces, +3:solid, +8:texnam is tiletype * instead of filename, +16 vertex shading 
*  @param  col				base color 24-bit hexadecimal color value (RGB) (0x404040) is the natural value
*
*		@example
*
...
*	Must be called between startFrame() & endFrame() functions.
*/
void VoxieBox::drawMeshExt(const char* fileName, point3d* VerticeXYZArray, int verticeXYZNum, uvcol_t* verticcUVCArray, int verticeUVCNum,  inds_t* indiceArray, int indiceNum, int flags, int col)
{
	voxie_drawmeshtex_ext(&vf, fileName, VerticeXYZArray, verticeXYZNum, verticcUVCArray, verticeUVCNum, indiceArray, indiceNum, flags, col);
}
 ...
 *  @param volumeCaptureMode the capture mode to use. Off (Stop recording) = 0, Single PLY = 1,  Video PLY = 2, Single PNG = 3, Single REC = 4, Video Rec = 5, Single VCB = 6, Video VCB = 7, Volume capture Net = 9;
 *  @param targetVPS if
...
void VoxieBox::updateKeyboardHistory()
{
	...
	for (i = 0; i < KEY_HISTORY_LENGTH; i++) {
		...
		if (voxie_keystat(keyHistory.history[i].inputCode) && keyHistory.history[i].isHeld == true) {
			...
			keyHistory.history[i].inputCode = 0;
			keyHistory.history[i].inputCodeRaw = 0;
			keyHistory.history[i].isHeld = false;
			keyHistory.history[i].startLastPressed = 0;
			keyHistory.history[i].startTime = 0;
		}
...
HINSTANCE VoxieBox::getVoxieBoxDLLHandle()
{
	return hvoxie;
}
